...
from core.gps import GPS, _parse_data, _parse_degrees, _parse_float, _parse_int, _parse_talker, _read_degrees

from ..typings.machine import UART

if TYPE_CHECKING:
    from ..typings._typeshed import AnyReadableBuf
...
def test_parse_data_unknown_sentence_type(sentence_type: Literal[-1, 10]):
...
class UartMock(UART):
    ...
    def write(self, bytestr: AnyReadableBuf):
        print(bytestr, end="")
...
def test_GPS_update_rmc_no_magnetic_variation():
    ...
    with mock.patch.object(GPS, "readline", return_value=r):
        ...
        assert gps._raw_sentence == "$GPRMC,215032.086,A,1234.5678,N,00123.12345,E,0.45,56.35,021021,,,A*6A"
        assert gps.nmea_sentence == "$GPRMC,215032.086,A,1234.5678,N,00123.12345,E,0.45,56.35,021021,,,A*6A"
...
def test_GPS_update_rmc_fix_is_set():
    ...
    with mock.patch.object(GPS, "readline", return_value=r_valid):
        gps = GPS(uart=UartMock())
        _extracted_from_test_GPS_update_rmc_fix_is_set_6(gps, 1, True)
    with mock.patch.object(gps, "readline", return_value=r_invalid):
        _extracted_from_test_GPS_update_rmc_fix_is_set_6(gps, 0, False)


# TODO Rename this here and in `test_GPS_update_rmc_fix_is_set`
def _extracted_from_test_GPS_update_rmc_fix_is_set_6(gps, arg1, arg2):
    assert gps.update()
    assert gps.fix_quality == arg1
    assert gps.has_fix is arg2
...
def test_GPS_update_rmc_debug_shows_sentence(capsys: pytest.CaptureFixture[str]):
    r = b"$GPRMC,215032.086,A,1234.5678,N,00123.12345,E,0.45,56.35,021021,,,A*6A\r\n"
    with mock.patch.object(GPS, "readline", return_value=r):
...
def test_GPS_send_command_with_checksum(capsys: pytest.CaptureFixture[str]):
    gps = GPS(uart=UartMock())
...
def test_GPS_send_command_without_checksum(capsys: pytest.CaptureFixture[str]):
    gps = GPS(uart=UartMock())
    gps.send_command(command=b"$PMTK001,314,3\r\n", add_checksum=False)
    out, err = capsys.readouterr()
    assert err == ""
...
def test_GPS_update_from_GGA():
    ...
    with mock.patch.object(GPS, "readline", return_value=r):
        ...
        assert gps.has_3d_fix is False
        assert gps.datetime == exp_time
        assert gps._raw_sentence == "$GPGGA,123519,4807.038,N,01131.000,E,1,08,0.9,545.4,M,46.9,M,,*47"
        assert gps.nmea_sentence == "$GPGGA,123519,4807.038,N,01131.000,E,1,08,0.9,545.4,M,46.9,M,,*47"
...
def test_GPS_update_from_GSA(
    r: Literal[
        b"$GPGSA,A,3,15,18,14,,,31,,,23,,,,04.5,02.1,04.0*0f\x26",
        b"$GPGSA,A,3,15,18,14,,,31,,,23,,,,04.5,02.1,04.0,3*\x26",
    ]
):
    with mock.patch.object(GPS, "readline", return_value=r):
        gps = GPS(uart=UartMock())
...
def test_GPS_update_from_GSV_first_part():
    ...
    with mock.patch.object(GPS, "readline", return_value=r):
        ...
        assert gps._raw_sentence == "$GPGSV,2,1,08,01,40,083,46,02,17,308,41,12,07,344,39,14,22,228,45*75"
        assert gps.nmea_sentence == "$GPGSV,2,1,08,01,40,083,46,02,17,308,41,12,07,344,39,14,22,228,45*75"
