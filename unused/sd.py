...
from .utils import spi_device
from .utils.typing import TYPE_CHECKING
...
class SDCard:
    ...
    async def initialise(self) -> None:
        # initialise the card
        assert self._spi.pin is not None
        await self._init_card(self._spi.pin)

        # Create a new SPIDevice with the (probably) higher operating baudrate.
        self._spi = spi_device.SPIDevice(self._spi.spi, self._spi.pin, baudrate=self.baudrate, extra_clocks=8)

    async def _init_card(self, chip_select: Pin) -> None:
        """Initialize the card in SPI mode."""
        # clock card at least 80 cycles with cs high
        async with self._spi as card:
            # Force CS high.
            chip_select.on()
        ...
        async with self._spi as card:
            ...
            if r == _R1_IDLE_STATE:
                await self._init_card_v2(card)
            elif r == (_R1_IDLE_STATE | _R1_ILLEGAL_COMMAND):
                await self._init_card_v1(card)
            else:
    ...
    async def _init_card_v1(self, card: SPI) -> None:
    ...
    async def _init_card_v2(self, card: SPI) -> None:
    ...
    def _wait_for_ready(self, card: SPI, timeout: float = 0.3) -> None:
        ...
        while time.time() - start_time < timeout and self._single_byte[0] != 0xFF:
            card.readinto(self._single_byte, 0xFF)

    def _cmd(
        self,
        card: SPI,
        cmd: int,
        arg: int | ReadableBuf = 0,
        crc: int = 0,
        response_buf: WritableBuf | None = None,
            ...
            if not (buf[0] & 0x80):
                if response_buf:
                    if data_block:
                        ...
                        buf[1] = 0xFF
                        for _ in range(_CMD_TIMEOUT):
                            card.readinto(buf[1:2], 0xFF)
                            if buf[1] == 0xFE:
                                break
                        if buf[1] != 0xFE:
                            return -1
                    card.readinto(response_buf, 0xFF)
                    if data_block:
                        # Read the checksum
                        card.readinto(buf[1:3], 0xFF)
                return buf[0]
        return -1
    ...
    def _block_cmd(
        ...
        if response_buf is not None and result == 0:
    ...
    def _readinto(self, card: SPI, buf: WritableBuf, start: int = 0, end: int | None = None) -> None:
        """
        ...
        while buf[start] != 0xFE:
            card.readinto(buf[start : start + 1], 0xFF)

        card.readinto(buf[start:end], 0xFF)

        # read checksum and throw it away
        card.readinto(self._cmdbuf[:2], 0xFF)
    ...
    def readblocks(self, start_block: int, buf: WritableBuf) -> int:
        """
    ...
    def writeblocks(self, start_block: int, buf: bytearray) -> int:
        """
...
def _calculate_crc_table() -> bytearray:
    ...
    crc_poly = 0x89  # the value of our CRC-7 polynomial
...
def calculate_crc(message: ReadableBuf) -> int:
    """
